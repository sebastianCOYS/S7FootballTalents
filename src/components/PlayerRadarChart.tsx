...
export default function PlayerRadarChart({ player, chartType }: PlayerRadarChartProps) {
    if (chartType === "offensive") {
        return <RadarChart sx={{ minWidth: "300px" }} height={300}
            series={[{ label: player.Player, data: [player.Gls, player["G-PK"], player.Ast, player.xG, player.npxG, player.xAG, player["G+A"], player.Carries, player.PrgP, player.PrgC, player.KP, player.Fld_stats_misc, player.Crs], fillArea: true },]}
    ...
    if (chartType === "goalkeeping") {
        return <RadarChart sx={{ minWidth: "300px" }} height={300}
            series={[{ label: player.Player, data: [Number(player.GA), Number(player.GA90), Number(player.Saves), Number(player["Save%"]), Number(player.CS), Number(player.PKsv), Number(player["Cmp%"])], fillArea: true },]}
